...
int at24_SequentialRead(I2C_HandleTypeDef *hi2c ,uint8_t DevAddress,uint16_t MemAddress,uint8_t *pData,uint16_t RxBufferSize);
int at24_SequentialWrite(I2C_HandleTypeDef *hi2c ,uint8_t DevAddress,uint16_t MemAddress,uint8_t *pData,uint16_t TxBufferSize);


int EEPROM_WriteBytes(uint16_t MemAddress, uint8_t *pData,uint16_t TxBufferSize){
	return at24_SequentialWrite(EEPROM_I2C_PTR, EEPROM_DEVICE_ADDR, MemAddress, pData, TxBufferSize);
}
int EEPROM_ReadBytes(uint16_t MemAddress, uint8_t *pData,uint16_t RxBufferSize){
	return at24_SequentialRead(EEPROM_I2C_PTR, EEPROM_DEVICE_ADDR, MemAddress, pData, RxBufferSize);
}
